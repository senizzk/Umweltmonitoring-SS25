...
# Dash-App initialisieren mit Bootstrap-Theme 
...
app.layout = dbc.Container([
    ...
    # Erste Zeile mit zwei Hauptkarten
    dbc.Row([
        # Linke Spalte (4/12 Breite) - SenseBox Info Card
        dbc.Col(
            sensebox_info_card(), 
            md=4,   
            class_name="h-100"
        ),
        # Rechte Spalte (8/12 Breite) - Verschachtelte Karten
        dbc.Col(
            nested_cards(), 
            md=8, 
            class_name="h-100"
    ...
    # Zweite Zeile mit Temperatur-Prognose-Karte
    dbc.Row([
        dbc.Col(temperatur_prognose_card())
    ], class_name="mb-4"),

    # Dritte Zeile mit Verlaufsgraph
    dbc.Row([
    dbc.Col(verlauf_graph_card())
    ], class_name="mb-5"),

    # Countdown-Anzeige (fixiert in der unteren linken Ecke)
...
# Callbacks initialisieren (aus separater Datei importiert)
